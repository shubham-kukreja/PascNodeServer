...
router.put("/update/:id", isAuthenticated, async (req, res) => {
  ...
  if (id != author)
    return res.status(401).send("You are not allowed to update this blog");
...
router.put("/reviewblogs/approve/:id", async (req, res) => {
...
router.delete("/delete/:id", async (req, res) => {
  try {
    ...
    const blog = await Blog.findById(req.params.id);
    if (!blog)
      return res.status(400).send("Blog with given id does not exists");
...
router.delete("/admin/delete/:blogid", async (req, res) => {
  try {
    console.log(req.params.blogid);
